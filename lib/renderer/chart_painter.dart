...
//    }
  ...
  void drawCrossLineText(Canvas canvas, Size size) {
    ...
    if (y > size.height - mBottomPadding) {
      y = size.height - mBottomPadding;
    }
    if (isInSecondaryRect(Offset(selectX, y))) return;
    TextPainter tp;
...
//For TrendLine
  ...
  void drawCrossLine(Canvas canvas, Size size) {
    ...
    final max = size.height - mBottomPadding;
    final space = this.chartStyle.nowPriceLineSpan + this.chartStyle.nowPriceLineLength;
    while (startY < max) {
      canvas.drawLine(
          Offset(x, startY), Offset(x, startY + this.chartStyle.nowPriceLineLength), paintY);
      startY += space;
    }
    ...
    // k线图横线
    if (!isInSecondaryRect(Offset(selectX, y))) {
      double startX = -mTranslateX;
      final max = -mTranslateX + mWidth / scaleX;
      final space = this.chartStyle.nowPriceLineSpan + this.chartStyle.nowPriceLineLength;
      while (startX < max) {
        canvas.drawLine(
            Offset(startX, y), Offset(startX + this.chartStyle.nowPriceLineLength, y), paintX);
        startX += space;
      }
  ...
  double getMainYValue(double y) => mMainRenderer.getYValue(y);
